import Link from "next/link";
import { site } from "@/lib/site";

const socialIcons: { name: string; url: string; path: string }[] = [
  {
    name: "Instagram",
    url: site.social.instagram,
    path: "M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zm0 10.162a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z",
  },
  {
    name: "TikTok",
    url: site.social.tiktok,
    path: "M12.53.02C13.84 0 15.14.01 16.44 0c.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z",
  },
  {
    name: "LinkedIn",
    url: site.social.linkedin,
    path: "M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 1 1 0-4.124 2.062 2.062 0 0 1 0 4.124zM7.119 20.452H3.554V9h3.565v11.452z",
  },
  {
    name: "YouTube",
    url: site.social.youtube,
    path: "M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z",
  },
];

const legalLinks = [
  { label: "Privacy Policy", href: "/privacy" },
  { label: "Terms of Use", href: "/terms" },
  { label: "Program Terms", href: "/program-terms" },
  { label: "Cookies", href: "/cookies" },
];

export default function Footer() {
  return (
    <footer className="border-t-4 border-ink bg-ink text-paper">
      {/* Final call */}
      <div className="border-b-4 border-paper px-4 py-14 md:px-8 md:py-20">
        <h2 className="mb-6 max-w-4xl text-3xl font-black uppercase leading-none tracking-tight sm:text-5xl md:mb-8 md:text-6xl">
          Got an idea? <span className="text-strike">Bring it.</span>
        </h2>
        <Link
          href="/apply"
          className="inline-block bg-strike px-8 py-4 text-base font-bold uppercase tracking-wide text-paper transition-colors hover:bg-paper hover:text-ink"
        >
          Apply to the Incubator
        </Link>
      </div>

      {/* Contact grid */}
      <div className="grid gap-10 px-4 py-12 md:grid-cols-3 md:gap-6 md:px-8 md:py-16">
        <div>
          <img src="/logo.svg" alt="Elan Innovate" className="mb-4 h-10 w-auto" />
          <p className="text-sm font-bold uppercase tracking-widest opacity-80">
            {site.endorsement}
          </p>
        </div>
        <div>
          <p className="mb-3 text-xs font-bold uppercase tracking-widest text-strike">
            Contact
          </p>
          
          <a  href={`mailto:${site.email}`}
            className="block text-sm font-medium leading-loose hover:text-strike"
          >
            {site.email}
          </a>
          
          <a  href={`tel:${site.phoneRaw}`}
            className="block text-sm font-medium leading-loose hover:text-strike"
          >
            {site.phone}
          </a>
        </div>
        <div>
          <p className="mb-3 text-xs font-bold uppercase tracking-widest text-strike">
            Follow
          </p>
          <div className="flex gap-3">
            {socialIcons.map((social) =>
              social.url ? (
                <a
                  key={social.name}
                  href={social.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label={`Elan Innovate on ${social.name}`}
                  className="border-2 border-paper p-3 transition-colors hover:border-strike hover:bg-strike"
                >
                  <svg
                    viewBox="0 0 24 24"
                    className="h-5 w-5 fill-current"
                    aria-hidden="true"
                  >
                    <path d={social.path} />
                  </svg>
                </a>
              ) : (
                <span
                  key={social.name}
                  title={`${social.name} coming soon`}
                  className="cursor-not-allowed border-2 border-paper p-3 opacity-40"
                >
                  <svg
                    viewBox="0 0 24 24"
                    className="h-5 w-5 fill-current"
                    aria-hidden="true"
                  >
                    <path d={social.path} />
                  </svg>
                </span>
              )
            )}
          </div>
        </div>
      </div>

      {/* Legal links */}
      <div className="flex flex-wrap gap-x-6 gap-y-2 border-t-4 border-paper px-4 py-5 text-xs font-bold uppercase tracking-widest text-paper/60 md:px-8">
        {legalLinks.map((link) => (
          <Link key={link.href} href={link.href} className="hover:text-paper">
            {link.label}
          </Link>
        ))}
      </div>

      {/* Bottom bar */}
      <div className="border-t-4 border-paper px-4 py-5 md:px-8">
        <p className="text-xs font-medium uppercase tracking-widest opacity-70">
          &copy; {new Date().getFullYear()} {site.name}. All rights reserved.
        </p>
      </div>
    </footer>
  );
}